...
class GetSubjectTutorsAction extends Action
{
    public function run()
    {
        $subjectTutors = Subject::join('subject_tutor', 'subject_tutor.subject_id', '=', 'subjects.id')
            ...
            ->join('adminstrators', 'adminstrators.id', '=', 'subject_tutor.assigned_by')
            ->join('departments','departments.id','=','subjects.department_id')
            ->join('classroom','classroom.id','=','subjects.class_id')
            ->select('tutors.first_name as tutor_first_name', 'tutors.last_name as tutor_last_name', 'adminstrators.full_name as assigned_by', 'subjects.name as subject_name', 'subjects.code as subject_code','departments.name as department_name','classroom.name as class_name')
            ->get();
